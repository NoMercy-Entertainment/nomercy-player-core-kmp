...
class FullSurfaceDivergenceTest {
    ...
    fun pinningMeansAnExactTrackOnTwoEnginesAndACeilingOnApple() {
        ...
        // will notice. Media3 and libVLC select a variant; AVFoundation caps a
    ...
    fun onlyMedia3CanTellHdrFromSdr() {
        // Media3 reads the transfer function. libVLC exposes none through its
        // track info, so every desktop rung reports SDR — which means an HDR
        // ladder on the desktop is indistinguishable from an SDR one, and the
        // HDR constraint has nothing to act on there.
    ...
    fun theHdrConstraintOnlyExistsWhereTheRangeIsKnown() {
        ...
        val asDesktopSeesIt: List<QualityLevel> = mixedLadder.map {
            it.copy(dynamicRange = VlcTrackMapper.dynamicRange())
        }
    ...
    fun codecNamesReachTheSameFamilyFromThreeVocabularies() {
        ...
        // platforms. libVLC says h264 and hevc; a manifest says avc1.640028 and
        // hvc1.2.4.L153.B0; the library compares families.
    ...
    fun everyEngineReportsAnUnknownAsSomethingSortable() {
        // Each engine has its own sentinel for "I do not know yet" — libVLC's -1
        // on time, length and volume, Media3's NO_VALUE, AVFoundation's NaN — and
        // each one sorts a ladder or draws a scrubber wrongly if it escapes.
        // libVLC's is the one visible from here; Apple's NaN is asserted in

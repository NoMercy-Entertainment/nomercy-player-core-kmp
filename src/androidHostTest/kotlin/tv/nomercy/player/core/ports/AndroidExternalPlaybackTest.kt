...
// Android, where the platform has its own casting and no AirPlay sender.
//
...
// belongs on screen.
class AndroidExternalPlaybackTest {

    @Test
    fun thisPlatformSaysItHasNowhereToSendPlaybackTo() {
        val external: ExternalPlayback = UnsupportedExternalPlayback

        assertFalse(external.isSupported, "Android claimed an external route sender")
